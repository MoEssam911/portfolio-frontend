      ...
      <span
        :class="
          cn(
            ...
            entry.current ? 'bg-primary shadow-glow' : 'bg-primary/35',
          )
        "
        aria-hidden="true"
      />
      <!-- Neon connector — lime gradient fading downward. -->
      <span
        v-if="!last"
        class="absolute top-1.5 left-1/2 z-0 h-full w-px -translate-x-1/2 bg-linear-to-b from-primary/45 via-primary/15 to-transparent"
        aria-hidden="true"
      />
    </div>
